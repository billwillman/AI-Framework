...
public unsafe class SffLoader
{
    ...
    public List<Texture> GetTextures()
    {
        if (!mIsVaild)
            return null;
        List<Texture> ret = null;
        var iter = mPcxDataMap.GetEnumerator();
        while (iter.MoveNext())
        {
            if (ret == null)
                ret = new List<Texture>();
            if (iter.Current.Value.Key.IsPng)
            {
                
                Texture2D tex = new Texture2D(1, 1, TextureFormat.ARGB32, false, false);
                tex.LoadImage(iter.Current.Value.Value.data);
            ...
            {
                Texture2D tex = new Texture2D(iter.Current.Value.Key.widht, iter.Current.Value.Key.height, TextureFormat.Alpha8, false);
                tex.filterMode = FilterMode.Point;
                tex.wrapMode = TextureWrapMode.Clamp;
                tex.LoadRawTextureData(iter.Current.Value.Value.data);
                tex.Apply(false, true);
                ret.Add(tex);
            }
        }
